...
plot_transition_function <- function(results_nl,specs) {
  ...
  date <- date[(max(c(specs$lag_endog,specs$lag_exog))):(length(date)-1)]
  length(date)
  ...
  state_plot <- ggplot(df_state)  + 
    ...
    scale_y_continuous(name = "",
                       ...
                       expand = c(0.01, 0.01)) +
    labs(title = 'Transition function for the state of currency appreciation') +
  ...
  ggsave(paste0('State_',regime_2,'.png'),
         ...
         device = "png",width = 12, height = 6, units = "cm",scale = 2,
         bg = 'white') 
}
...
plot_IRFs <- function(results_nl,specs) {
  ...
  for (i in 1:specs$n_endo_variables) {
    ...
    plot_lst_nl_s1[[i]] <- ggplot(IRF_df %>% select(matches('*s1$'),horizon))  + 
      ...
      scale_y_continuous(name = "",
                         ...
                         expand = c(0, 0),
                         labels = function (x) sprintf("%0.4f", round(x, digits = 4))) +
  ...
  legend_plot_1 <- ggplot(IRF_df2) +
    ...
    scale_fill_manual(name = 'State', labels = c('Depreciation',
                                                 'Appreciation'),
                      values = c('indianred', 
                                 'lightblue'),guide = 'legend') +
...
plot_Belaisch_ERPT <- function(results_lin,results_nl,specs) {
  ...
  RC_plot <- ggplot(RC_df)  + 
    ...
    geom_line(data = RC_df %>% select(horizon,RC_lin, RC_s2,RC_s1) %>%
                pivot_longer(cols = c(everything(),-horizon)),
      aes(x=horizon, y = value, color = name, linetype = name), size = 2) +
    ...
    # Legendas
    scale_color_manual(name = 'State', values = c('darkgreen',
                                                  'indianred',
                                                  'dodgerblue'),
                       labels = c('Depreciation',
                                  'Appreciation',
  ...
  legend_plot_1 <- ggplot(IRF_df2) +
    geom_line(aes(x=horizon, y=value, color = name, linetype = name),size = 1.5, show.legend = T) +
    scale_color_manual(name = 'State', values = c('indianred',
                                                  'dodgerblue',
                                                  'darkgreen'),
                       labels = c('Depreciation',
  ...
  plot_grid(RC_plot,
            ...
            nrow = 2,
            rel_heights =  c(2.5,1)) +
    draw_grob(legend_plot_1, 1/10, -0.10, 1/3, 0.5) +
    draw_grob(legend_plot_2, 6/10, -0.10, 1/3, 0.5) +
    theme_minimal() +
    theme(  panel.grid = element_blank(),
            panel.border = element_blank())
